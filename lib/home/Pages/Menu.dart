...
class _MenuState extends State<Menu> {
  final List<String> categories = ['Tous', 'Entrées', 'Plats', 'Desserts', 'Boisson'];
  ...
  void initState() {
    super.initState();
    _dishesFuture = fetchDishesFromJson();
  }

  Future<List<Map<String, dynamic>>> fetchDishesFromJson() async {
    final String response = await rootBundle.loadString('assets/data/plats.json');
    final List<dynamic> data = json.decode(response);
    List<Map<String, dynamic>> plats = List<Map<String, dynamic>>.from(data);

    if (selectedCategory == 'Tous') {
      return plats;
    } else {
      return plats.where((plat) => plat['categorie'] == selectedCategory).toList();
    }
  }
                                ...
                                children: [
                                  ...
                                  Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Text(plat['nom'], style: KTypography.h4(context)),
                                        Text("${plat['prix']} FCFA", style: KTypography.h4(context, color: KColors.primary)),
                                      ],
                                    ),
                                  ),
                                  const Spacer(flex: 7),
                                    ...
                                    child: ElevatedButton.icon(
                                      onPressed: () {
                                        ...
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(
                                            content: Text("${plat['nom']} ajouté à la commande"),
